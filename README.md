...
透明弹幕保留国际新闻，新增财经来源。右上角可选择“综合 / 财经 / 国际”，此选择同步新闻列表并记住偏好。列表支持“重点优先 / 最新发布”，显示主题、来源、原始发布时间和可点击原文。仅展示标题，不转载正文。快照目前每源限额合计最多 35 条，存储上限 48 条；综合弹幕精选最多 8 条财经和 6 条国际，完整内容在新闻列表查看。浏览器直连时华尔街见闻按发布时间保留最近 16 条，合并后最多 48 条。
...
英文新闻标题在定时抓取时先通过 Google 翻译公共接口转换为简体中文，再进入飘动栏和新闻列表。按完整原始标题复用已发布译文；标题变更会重新翻译。悬停可查看英文原题，点击仍进入来源文章。翻译失败的英文标题暂不展示，下次定时更新时重试；浏览器旧缓存中的未译标题也会过滤。AI、ETF 等中文标题中的常用缩写保留。翻译仅发送公开新闻标题，不发送用户命令或本地数据。公共翻译接口可能不可用或限流。
...
新增数据源在构建端获取，除华尔街见闻支持浏览器直连外，其余来源通过同站快照读取；无需用户提供密钥。公共接口可能调整，失败时不以空数据覆盖旧新闻。有效但近期没有公告的央行 RSS 视为正常空源，不长期报成连接故障。

鼠标悬停或点击“暂停”可停止滚动；“新闻列表”提供完整标题、发布时间和来源链接，也方便键盘访问。在系统开启“减少动态效果”时自动改为静态横向浏览。

点击“刷新”会同时请求华尔街见闻公开快讯接口和网站的全源新闻快照。财经快讯无需等待网站重新部署，浏览器每 5 分钟自动检查，回到页面时也会检查。源站允许跨域读取，请求不携带凭据；来源故障、网络限制或无有效标题时保留旧内容并提示失败。只显示源站提供的标题和原始发布时间，不用刷新时间替代新闻时间。

状态栏分别显示“财经直连”的最近成功检查时间和“全源”的实际抓取时间；财经更新不会把其余来源标成刚刚更新。没有新标题时更新时间但保持弹幕位置，默认按最新发布排列，可切换重点优先。

GitHub Actions 按每小时第 7、22、37、52 分钟计划抓取全部来源并部署。GitHub 定时任务可能延迟或漏执行，不能保证每 15 分钟准时更新；全源超过 3 小时未抓取会显示延迟。国际及其他财经来源仍依赖这份快照，“刷新”不触发 GitHub Actions。网站新闻数据更新不会修改浏览器命令库。
...
- `live-news.js`：财经直连、标题与链接校验、保留来源发布时间。
- `tests/news-direct.test.cjs`：直连刷新、过期快照提示、缓存与失败保留的浏览器回归测试，需要 Playwright。
...
```bash
node --test tests/model.test.cjs tests/live-news.test.cjs
