...
(async()=>{
 ...
 try {
  ...
  assert.equal(calls,1);assert.match(await page.locator('#news-updated').getAttribute('title'),/财经直连 已检查.*全源 更新延迟/);
  ...
  snapshot={...base,fetchedAt:new Date(Date.now()-60000).toISOString()};await click();assert.match(await page.locator('#news-updated').getAttribute('title'),/全源 更新 /);
